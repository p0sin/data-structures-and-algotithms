...
class hash_table():
    ...
    def get(self,key): #Function to return the value of the key entered by the user
        hash = self._hash(key) #Hash value of the key is calculated by passing the key to the _hash function
...
#Also, the for loop in the get function runs for the length of the collision array, which in most cases, won't be too long
#At least nowhere near to the number of total entries, hence the time complexity remains way less than O(n), even less than O(log n) in most cases
