...
#Then we will make the 'next' of the new node point to the head ,as the head is currently pointing to the first node of the list
#And then we will update the head to point to new node as we want the new node to be new first node, i.e, the new head.
#And of course, we'll increase the length by 1
...
#Since this requires us to traverse the entire length og the linked list, this is an O(n) operation.
    def print_list(self):
        if self.head == None:
            print("Empty")
        else:
...
#If the position is somewhere in between, then we create a temporary node which traverses the list upto the previous position of the position we want to enter the new node
#Now the 'next' of the temporary node is pointing to the next node in the list, where we want to insert our new node
...
#(If the value is found multiple times, only the first occurrence of the value will be deleted.)
